...
private:
	...
	void AlignNodesSize(const CString& strSize);
	void AlignNodesSide(const CString& side);
	void SelectAll();
	...
	void TrackMultiple(CPoint& logPt, CPoint& point, CDC* pDC);
	void TrackSingle(CPoint& logPt, CPoint& point, CDC* pDC, BOOL keepRatio = FALSE);
	void DrawLinkAdding(CDC* pDC);
